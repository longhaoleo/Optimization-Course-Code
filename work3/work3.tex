...
\begin{itemize}
  ...
  \item 采用 Armijo 回溯步长搜索，实现 BFGS（代码中函数名为 \texttt{bfgs}）与 FR 共轭梯度法（\texttt{fr}），求解 Powell 奇异函数。
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{optimization/optimizers/bfgs.py}：BFGS 优化器实现；
...
\textbf{代码展示 3：bfgs 与 FR 算法核心迭代}
\begin{lstlisting}[caption={bfgs.py 中 BFGS 主迭代代码},label={lst:bfgs-impl}]
def bfgs(
    x0: np.ndarray,
    objective: Objective,
...
Powell & bfgs + Armijo & $\epsilon$, max\_iter & $10^{-6}$, $20000$ \\
...
\subsection{Armijo + bfgs/FR（Powell 奇异函数）}
\begin{table}[H]
\centering
...
方法 & 迭代次数 & $f^\ast$ & 是否收敛 & $\|\nabla f(x^\ast)\|$ \\
\midrule
bfgs + Armijo & 29 & $3.2600\times 10^{-10}$ & 是 & $7.3476\times 10^{-7}$ \\
